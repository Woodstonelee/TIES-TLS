...
bool CppArcInfo(vector<double> x, vector<double> y, vector<double> circle, double scanpos_x, double scanpos_y, vector<bool> &InSemiCircle, vector<double> &ArcInfo)
{
	...
	int MaxBlankDivCount = count(BlankDivs.begin(), BlankDivs.end(), *MaxBlankDiv);
	if (*MaxBlankDiv < 2)
	{
		ArcInfo[0] = 0;
		ArcInfo[1] = 2*PI;
	...
	else
	{
		...
		else
		{
			...
			ArcInfo[2] += AngleResolution * (StartAngleDiv[MaxInd] + 1) - ArcInfo[0];
...
double CppNeighborPtsDensity(vector<unsigned int> edgeImage, vector<unsigned int> dim_img, 
							 unsigned int windowsize, double rowc, double colc)
{
	...
	if ( (rowc>static_cast<double>(dim_img[0])) || (rowc<0) || (colc>static_cast<double>(dim_img[1])) || (colc<0) )
	{
		return mxGetInf();
	}

	for (int c=-1*relativecenter; c<static_cast<int>(relativecenter+1); c++)
	{
		...
		for (int r=-1*relativecenter; r<static_cast<int>(relativecenter+1); r++)
		{
			// if window pixel locates outside the image, its point number is assumed to be 0.
			if ( ((rowc+r)>static_cast<double>(dim_img[0])) || ((rowc+r)<0) )
			{
				continue;
			}
			meanpnum += edgeImage[ MatrixLZ::sub2ind(static_cast<unsigned int>(rowc+r), static_cast<unsigned int>(colc+c), dim_img[0], dim_img[1], MatrixLZ::ColMajor) ];
		}
	}
...
bool CppMarkROI(vector<unsigned int> edgeImage, vector<unsigned int> dim_img, 
				vector<double> oneHTcircle, double scanpos_x, double scanpos_y,  
				vector<double> xpix, vector<double> ypix, 
				unsigned int sigmacoef, double Rsigma, 
				vector<bool> &ROIimage, vector<double> &ROIxpix, vector<double> &ROIypix, 
				vector<double> &OHCROIxpix, vector<double> &OHCROIypix)	// OHC: Outside Half Circle
{
	...
	if (dim_img.size()!=2 || xpix.size()!=ypix.size() || oneHTcircle.size()!=3)
	{
#ifdef MATLABPRINT
	...
	maxcol=maxcol<static_cast<int>(dim_img[1]-1) ? maxcol:(dim_img[1]-1);
	maxcol=maxcol>0 ? maxcol:0;
	for (unsigned int cind=static_cast<unsigned int>(mincol); cind<static_cast<unsigned int>(maxcol+1); cind++)
	{
		minrow=static_cast<int>( rowc-sqrt( maxR*maxR-(colc-cind)*(colc-cind) ) );
		maxrow=static_cast<int>( rowc+sqrt( maxR*maxR-(colc-cind)*(colc-cind) ) );
		minrow=minrow<0 ? 0:minrow;
		minrow=minrow<static_cast<int>(dim_img[0]-1) ? minrow:(dim_img[0]-1);
		maxrow=maxrow<static_cast<int>(dim_img[0]-1) ? maxrow:(dim_img[0]-1);
		maxrow=maxrow>0 ? maxrow:0;
		for (unsigned int rind=static_cast<unsigned int>(minrow); rind<static_cast<unsigned int>(maxrow+1); rind++)
		{
			unsigned int temp;
			temp=MatrixLZ::sub2ind(rind, cind, dim_img[0], dim_img[1], MatrixLZ::ColMajor);
			ROIimage[ MatrixLZ::sub2ind(rind, cind, dim_img[0], dim_img[1], MatrixLZ::ColMajor) ]=true;
		}
	}
	...
	for (unsigned int rind=static_cast<unsigned int>(minrow); rind<static_cast<unsigned int>(maxrow+1); rind++)
	{
		mincol=static_cast<int>( colc-sqrt( maxR*maxR-(rowc-rind)*(rowc-rind) ) );
		maxcol=static_cast<int>( colc-octlen );
		mincol=mincol<0 ? 0:mincol;
		mincol=mincol<static_cast<int>(dim_img[1]-1) ? mincol:(dim_img[1]-1);
		maxcol=maxcol<static_cast<int>(dim_img[1]-1) ? maxcol:(dim_img[1]-1);
		maxcol=maxcol>0 ? maxcol:0;
		for (unsigned int cind=mincol; cind<static_cast<unsigned int>(maxcol+1); cind++)
		{
			ROIimage[ MatrixLZ::sub2ind(rind, cind, dim_img[0], dim_img[1], MatrixLZ::ColMajor) ]=true;
		}
		mincol=static_cast<int>( colc+octlen );
		maxcol=static_cast<int>( colc+sqrt( maxR*maxR-(rowc-rind)*(rowc-rind) ) );
		mincol=mincol<0 ? 0:mincol;
		mincol=mincol<static_cast<int>(dim_img[1]-1) ? mincol:(dim_img[1]-1);
		maxcol=maxcol<static_cast<int>(dim_img[1]-1) ? maxcol:(dim_img[1]-1);
		maxcol=maxcol>0 ? maxcol:0;
	...
	{
		dis=sqrt((xpix[i]-colc)*(xpix[i]-colc) + (ypix[i]-rowc)*(ypix[i]-rowc));
	...
	{
		if ( arrow2scanpos[0]*(initialROIxpix[i]-oneHTcircle[1])+arrow2scanpos[1]*(initialROIypix[i]-oneHTcircle[0]) > 0.0 )
		{
...
double CppRsigma(vector<unsigned int> edgerow, vector<unsigned int> edgecol, 
			   unsigned int rowc, unsigned int colc)
{
	...
	{
		dis[i]=(edgerow[i]-rowc)*(edgerow[i]-rowc) + (edgecol[i]-colc)*(edgecol[i]-colc);
		dis[i]=sqrt(dis[i]);
	}
	double meandis=0.0;
...
double CppSelectHTCircle(vector<unsigned int> edgeImage, vector<unsigned int> dim_img, 
					   vector<unsigned int> HTrow, vector<unsigned int> HTcol, vector<unsigned int> HTr, 
					   vector<double> &finalHTcircle, vector<double> &allRsigma)
{
	// HTrow, HTcol finalHTcircle([rowc, colc, r], in unit of pixel), based from 0.
	if (HTrow.size()!=HTcol.size() || HTcol.size()!=HTr.size() || HTrow.size()!=HTr.size())
	{
#ifdef MATLABPRINT
	...
	for (unsigned int i=0; i<edgeImage.size(); i++)
	{
		if (edgeImage[i])
		{
			currentrow=MatrixLZ::ind2subrow(i, dim_img[0], dim_img[1], MatrixLZ::ColMajor);
			currentcol=MatrixLZ::ind2subcol(i, dim_img[0], dim_img[1], MatrixLZ::ColMajor);
...
void CppRasterCircle(unsigned int x0, unsigned int y0, unsigned int radius, vector<unsigned int> &I, vector<unsigned int> &dim_I)
{
	...
	while(x < y)
	{
		...
		ddF_x += 2;
		f += ddF_x;    
		if (x0+x<dim_I[1]+1 && y0+y<dim_I[0]+1 && x0+x>0 && y0+y>0)
		{
		...
		if (x0+y<dim_I[1]+1 && y0+x<dim_I[0]+1 && x0+y>0 && y0+x>0)
		{
...
bool CppIterCHT(vector<unsigned int> originImage, vector<unsigned int> dim_originImage, unsigned int maxR, 
				vector<unsigned int> &edgeImage, vector<unsigned int> &dim_edgeImage, 
				vector<unsigned int> &result_row, vector<unsigned int> &result_col, vector<unsigned int> &result_r, vector<unsigned int> &result_peak)
{
	...
	vector<unsigned int> rarray(edgeImage.size(), 0), peakarray(edgeImage.size(), 0);
	...
	for (unsigned int rcount=1; rcount<=maxR; rcount++)
	{
		...
		for (unsigned int pcount=0; pcount<edgeImage.size(); pcount++)
		{
			if (edgeImage[pcount]!=0)	// the pixel is on the edge
			{
				...
				currentrow=MatrixLZ::ind2subrow(pcount, dim_edgeImage[0], dim_edgeImage[1], MatrixLZ::ColMajor);
				currentcol=MatrixLZ::ind2subcol(pcount, dim_edgeImage[0], dim_edgeImage[1], MatrixLZ::ColMajor);
				CppRasterCircle(currentcol+1, currentrow+1, rcount, accumulator, dim_edgeImage);
				edgecellnum += 1;
			}
		}	
	...
	{
		if (peakarray[i]>=1*maxpeak && peakarray[i]>=static_cast<unsigned int>(0.1*edgecellnum+0.5))
		{
			currentrow=MatrixLZ::ind2subrow(i, dim_edgeImage[0], dim_edgeImage[1], MatrixLZ::ColMajor);
			currentcol=MatrixLZ::ind2subcol(i, dim_edgeImage[0], dim_edgeImage[1], MatrixLZ::ColMajor);
...
void CppCircleFitting(vector<double> &input_x, vector<double> &input_y, 
					  vector<double> &circle)
{
	...
	{
#ifdef MATLABPRINT
		mexPrintf("CppCircleFitting ==> input x and y are not the same size or they are empty!\n");
#endif
		circle.clear();
		return;
	}
	...
	meanx = accumulate(input_x.begin(), input_x.end(), 0.0) / static_cast<double>(input_x.size());
	meany = accumulate(input_y.begin(), input_y.end(), 0.0) / static_cast<double>(input_y.size());
	...
	mx_A=mxCreateDoubleMatrix(3, 3, mxREAL);pt_A=mxGetPr(mx_A);
	mx_B=mxCreateDoubleMatrix(3, 1, mxREAL);pt_B=mxGetPr(mx_B);

	vector<double> xx(n), yy(n), xy(n);
	transform(x.begin(), x.end(), x.begin(), xx.begin(), std::multiplies<double>());
	transform(y.begin(), y.end(), y.begin(), yy.begin(), std::multiplies<double>());
	transform(x.begin(), x.end(), y.begin(), xy.begin(), std::multiplies<double>());
	...
	pt_A[0]=sumx;pt_A[3]=sumy;pt_A[6]=static_cast<double>(n);
	pt_A[1]=sumxy;pt_A[4]=sumyy;pt_A[7]=sumy;
	pt_A[2]=sumxx;pt_A[5]=sumxy;pt_A[8]=sumx;
...
bool CppGridPts(vector<double> x, vector<double> y, double cellsize, unsigned int maxR, unsigned int minnumfit, 
				vector<unsigned int> &img, vector<unsigned int> &dim_img, 
				vector<double> &refmatrix, vector<unsigned int> &dim_refmatrix)
{
...
#endif
		img.clear();
		dim_img[0]=0;dim_img[1]=0;
		refmatrix.clear();
		dim_refmatrix[0]=0;dim_refmatrix[1]=0;
		return true;
	}
...
#ifdef DEBUG
		mexPrintf("CppGridPts ==> CppCircleFitting return empty, circle fitting failed with all the points!\n");
#endif
		img.clear();
		dim_img[0]=0;dim_img[1]=0;
		refmatrix.clear();
	...
	dim_img[0]=rownum; dim_img[1]=colnum;
	img.clear();
	...
	dim_refmatrix[0]=mxGetM(plhs[0]); dim_refmatrix[1]=mxGetN(plhs[0]);
	refmatrix.clear();
...
bool CppDetEstCircle3(vector<double> x, vector<double> y, 
					 ...
					 vector<double> &detectedcircle, vector<double> &mapfinalHTcircle, vector<double> &iterationdata, 
					 vector<double> &ArcInfo, 
					 vector<double> &finalx, vector<double> &finaly, 
					 vector<double> &TotalOHCx, vector<double> &TotalOHCy, 
					 vector<double> &DebugInfo)	// OHC: Outside Half Circle
{
	// based from 0.
	...
	{
#ifdef MATLABPRINT
		mexPrintf("CppDetEstCircle ==> input x and y do not have the same length!\n");
#endif
	...
	bool callflag;
	callflag=CppGridPts(x, y, cellsize, maxR, minnumfit, img, dim_img, refmatrix, dim_refmatrix);
	if (!callflag)
	{
#ifdef MATLABPRINT
...
#ifdef DEBUG
		mexPrintf("CppDetEstCircle ==> calling CppGridPts ==> less than minnumfit points in this section!\n");
#endif
		detectedcircle.clear();
		mapfinalHTcircle.clear();
		iterationdata.clear();
	...
	mx_refmatrix=mxCreateDoubleMatrix(dim_refmatrix[0], dim_refmatrix[1], mxREAL);
	...
	transform(img.begin(), img.end(), zeros.begin(), ROIimage.begin(), std::not_equal_to<double>());
	centerchange=sqrt( (estcircle[0]-precircle[0])*(estcircle[0]-precircle[0]) + (estcircle[1]-precircle[1])*(estcircle[1]-precircle[1]) );
	...
	while ( centerchange>epsion/cellsize || radiuschange>epsion/cellsize )
	{
		...
		transform(img.begin(), img.end(), ROIimage.begin(), inputImage.begin(), std::multiplies<unsigned int>());
		//int debugcount=0;
		...
		HTr.clear();
		HTpeak.clear();
		if ( !( CppIterCHT(inputImage, dim_img, maxR, edgeImage, dim_img, HTrow, HTcol, HTr, HTpeak)
			) )
		{
#ifdef MATLABPRINT
		...
		finalHTcircle.clear();
		allRsigma.clear();
		finalRsigma=CppSelectHTCircle(edgeImage, dim_img, HTrow, HTcol, HTr, finalHTcircle, allRsigma);
		if (finalRsigma<0)
		{
#ifdef MATLABPRINT
		...
		// mark ROI
		ROIimage.clear();
		if ( !( CppMarkROI(edgeImage, dim_img, finalHTcircle, ScanPos_xpix, ScanPos_ypix, xpix, ypix, sigmacoef, finalRsigma, ROIimage, ROIxpix, ROIypix, OHCROIxpix, OHCROIypix)
			) )
		{
#ifdef MATLABPRINT
		...
		{
#ifdef DEBUG
			mexPrintf("CppDetEstCircle ==> less than minnumfit points used to fit a circle!\n");
#endif
			fitflag=false;
			break;
		}
	...
	double ptsdensity;
	ptsdensity=CppNeighborPtsDensity(img, dim_img, 3, estcircle[0], estcircle[1]);
	if (ptsdensity<0)
	{
#ifdef MATLABPRINT
	...
	if (mxIsInf(ptsdensity))
	{
		...
		detectedcircle.clear();mapfinalHTcircle.clear();
		detectedcircle.resize(3, 0.0); mapfinalHTcircle.resize(3, 0.0);
		prhs[0]=mx_refmatrix;
		prhs[1]=mxCreateDoubleMatrix(2,1,mxREAL);
		prhs[2]=mxCreateDoubleMatrix(2,1,mxREAL);
		tempGetPr=mxGetPr(prhs[1]);
		tempGetPr[0]=estcircle[0] + 1;tempGetPr[1]=finalHTcircle[0] + 1;
		tempGetPr=mxGetPr(prhs[2]);
		tempGetPr[0]=estcircle[1] + 1;tempGetPr[1]=finalHTcircle[1] + 1;
		mexCallMATLAB(2, plhs, 3, prhs, "pix2map");
		tempGetPr=mxGetPr(plhs[0]);	//xc
		detectedcircle[1]=tempGetPr[0];mapfinalHTcircle[1]=tempGetPr[1];	// xc
		tempGetPr=mxGetPr(plhs[1]);	//yc
		detectedcircle[0]=tempGetPr[0];mapfinalHTcircle[0]=tempGetPr[1];	// yc
		detectedcircle[2]=estcircle[2]*cellsize;mapfinalHTcircle[2]=finalHTcircle[2]*cellsize;	// r
		mxDestroyArray(prhs[1]);
		mxDestroyArray(prhs[2]);
		mxDestroyArray(plhs[0]);
		mxDestroyArray(plhs[1]);
	...
	if ( !CppArcInfo(ROIxpix, ROIypix, estcircle, ScanPos_xpix, ScanPos_ypix, InSemiCircle, ArcInfo) )
	{
#ifdef MATLABPRINT
