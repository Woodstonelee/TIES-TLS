...
bool
CppRasterize2d (vector < double >x, vector < double >y,
                                        ...
                                        //vector<double> &xcol, vector<double> &yrow, vector<unsigned int> &cell_np)
{
  ...
  dim_img.assign (2, 0);
  dim_img[0] =
    static_cast < unsigned int >(floor ((maxy - miny) / celly)) + 1;
  dim_img[1] =
    static_cast < unsigned int >(floor ((maxx - minx) / cellx)) + 1;
  ...
  for (unsigned int i = 0; i < x.size (); i++) {
    ...
    IndexVector[i] =
      static_cast <
      int >(MatrixLZ::
            sub2ind (ypixel, xpixel, dim_img[0], dim_img[1],
                     MatrixLZ::ColMajor));
...
Tree *
CTCC_buildkdtree (double *reference, unsigned int npoints)
{
  //double *reference;
  ...
  Tree *kdtree;
  if ((kdtree =
       build_kdtree (reference, npoints, 2, index, npoints, 0)) == NULL) {
    delete[]index;
#ifdef DEBUG
    mexPrintf
      ("CTCC_buildkdtree ==> Not enough free memory to build k-D tree\n");
#endif
    return NULL;
  } else {
    kdtree->dims = 2;
...
bool
CppTreeClusterClassifier (vector < double >x, vector < double >y,
                          vector < double >z, double cellsize,
                          double CutoffDis, unsigned int minpoints,
...
//      }
  ...
  for (unsigned int i = 0; i < img_np.size (); i++) {
    ...
    if (treetable_x.size () == 0)       // no tree, then create the first tree.
    {
      ...
      if ((kdtree =
           CTCC_buildkdtree (reference, treetable_x.size ())) == NULL) {
#ifdef DEBUG
      ...
      run_queries (kdtree->rootptr, modelpoint, 1, kdtree->dims, &closesttree,
                   &dist, RETURN_INDEX);
...
#ifdef DEBUG
    mexPrintf
      ("CppTreeClusterClassifier ==> TtableFileName opened failed!\n");
#endif
    //free_tree(kdtree->rootptr);
    //free(kdtree);
    return false;
  ...
  for (unsigned int i = 0; i < treetable_x.size (); i++) {
    if (treetable_npoints[i] >= minpoints) {
      Ttablefstream << treetable_x[i] * cellsize +
        minx << "\t" << treetable_y[i] * cellsize +
        miny << "\t" << treetable_npoints[i] << "\t" << i + 1 << endl;
    }
  }
  Ttablefstream.close ();
  ...
  for (unsigned int pcount = 0; pcount < x.size (); pcount++) {
    ...
    if (treetable_npoints[tempindex] < minpoints) {
      ...
      arrTfstream << x[pcount] << "\t" << y[pcount] << "\t" << z[pcount] <<
        "\t" << tempindex + 1 << endl;
    }
  }
  arrTfstream.close ();
...
mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
  ...
  if (npoints != mxGetNumberOfElements (prhs[1])
      || npoints != mxGetNumberOfElements (prhs[2])
  ...
  if (!(CppTreeClusterClassifier (x, y, z,
