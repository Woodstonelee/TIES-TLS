...
bool
CppRasterize3d (double *x, double *y, double *z, size_t np,
                double cellx, double celly, double cellz,
                char *tmpfilename, double *IndexVector, double *minmax,
                double filtercellsize, char *demptsfilename,
                char *rasterinfofilename)
{
  ...
  for (size_t xcount = 0; xcount < static_cast < unsigned int >(xpixelnum);
       xcount++) {
    for (size_t pcount = 0; pcount < np; pcount++) {
      if (valarray_x[pcount] < (minx + xcount * cellx)
          || valarray_x[pcount] >= (minx + (xcount + 1) * cellx)) {
        continue;
      }
      ypixel =
        static_cast < int >(floor ((valarray_y[pcount] - miny) / celly));
      zpixel =
        static_cast < int >(floor ((valarray_z[pcount] - minz) / cellz));
    ...
    for (int i = 0; i < ypixelnum; i++) {
      for (int j = 0; j < zpixelnum; j++) {
        if (slicematrix[i][j] != 0) {
          tmpfstream << (i + 1) << "\t" << (xcount + 1) << "\t" << (j +
                                                                    1) << "\t"
            << slicematrix[i][j] << endl;
          slicematrix[i][j] = 0;
        }
      }
    }
  ...
  rasterinfofstream << minmax[0] << "\t" << minmax[2] << "\t" << minmax[4] <<
    endl;
  rasterinfofstream << minmax[1] << "\t" << minmax[3] << "\t" << minmax[5] <<
    endl;
  ...
  filterxpixelnum =
    static_cast < int >(floor ((maxx - minx) / filtercellsize)) + 1;
  filterypixelnum =
    static_cast < int >(floor ((maxy - miny) / filtercellsize)) + 1;
  ...
  for (size_t pcount = 0; pcount < np; pcount++) {
    currentypix =
      static_cast <
      int >(floor ((valarray_y[pcount] - miny) / filtercellsize));
    currentxpix =
      static_cast <
      int >(floor ((valarray_x[pcount] - minx) / filtercellsize));
  ...
  for (int i = 0; i < filterypixelnum; i++) {
    for (int j = 0; j < filterxpixelnum; j++) {
      if (xyplane[i][j] != numeric_limits < double >::infinity ()) {
        demptsfstream << (minx + j * filtercellsize +
                          filtercellsize * 0.5) << ", " << (miny +
                                                            i *
                                                            filtercellsize +
                                                            filtercellsize *
                                                            0.5) << ", " <<
...
mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
  ...
  if (!
      (CppRasterize3d
       (x, y, z, np, cellx, celly, cellz, tmpfilename, IndexVector, minmax,
        filtercellsize, demptsfilename, rasterinfofilename))) {
    switch (nlhs) {
    case 2:
      break;
    case 1:
