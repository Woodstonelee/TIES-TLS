...
bool
CppStemExtraction (double *x, double *y, double *z,
                   double minx, double miny, double minz,
                   double maxx, double maxy, double maxz,
                   double cellx, double celly, double cellz,
                   double *voxelarray_yrow, double *voxelarray_xcol,
                   double *voxelarray_zvertica, double *voxelarray_pnum,
                   double *indexvector, double zflag, double maxstemlength,
                   vector < double >dem, vector < unsigned int >dim_dem,
                   vector < double >deminfo, char *stemptsfile,
                   char *nonstemptsfile, size_t np, size_t voxelnum)
{
  ...
  if (sizeof (int) != 4) {
    mexErrMsgTxt
      ("Sorry the current version only support machines with 4-byte int type! We are working to fix this stupid bug...");
    return false;
  }

  if ((maxx - minx) > numeric_limits < int >::max () * cellx) {
    mexPrintf
      ("CppStemExtraction ==> x extend exceeds the limits: %f meter\n!",
       numeric_limits < int >::max () * cellx);
    return false;
  }
  if ((maxy - miny) > numeric_limits < int >::max () * celly) {
    mexPrintf
      ("CppStemExtraction ==> y extend exceeds the limits: %f meter\n!",
       numeric_limits < int >::max () * celly);
    return false;
  }
  if ((maxz - minz) > numeric_limits < int >::max () * cellz) {
    mexPrintf
      ("CppStemExtraction ==> z extend exceeds the limits: %f meter\n!",
  ...
  valarray <
    unsigned char
    >voxelarray_continuous ((xpixelnum * ypixelnum * zpixelnum / 8) + 1);
  voxelarray_continuous = 0;
  int index_byte, index_bit;
  int continuouscount = 0;
  currentyrow = voxelarray_yrow[0];
  currentxcol = voxelarray_xcol[0];
  barsection_pnum[static_cast < int >(voxelarray_zvertica[0] - 1)] =
    static_cast < int >(voxelarray_pnum[0]);
  ...
  for (size_t vcount = 1; vcount < voxelnum; vcount++) {
    ...
    if (currentyrow == nextyrow && currentxcol == nextxcol) {
      barsection_pnum[static_cast < int >(voxelarray_zvertica[vcount] - 1)] =
        static_cast < int >(voxelarray_pnum[vcount]);
    } else {
      for (size_t zcount = belownum;
           zcount < static_cast < unsigned int >(zpixelnum - abovenum);
           zcount++) {
        continuousflag = true;
        for (int rindex = belownum * (-1); rindex < abovenum + 1; rindex++) {
          continuousflag = continuousflag
            && (barsection_pnum[zcount + rindex]);
        }
        if (continuousflag) {
          for (int rindex = belownum * (-1); rindex < abovenum + 1; rindex++) {
            currentindex =
              (zcount + rindex) * ypixelnum * xpixelnum + (currentxcol -
                                                           1) * ypixelnum +
              currentyrow;
            ...
            voxelarray_continuous[index_byte] =
              voxelarray_continuous[index_byte] | (1 <<
                                                   ((index_bit - 1 + 8) % 8));
          }
  ...
  if (maxstemlength == mxGetInf ()) {
    for (size_t pcount = 0; pcount < np; pcount++) {
      ...
      if ((voxelarray_continuous[index_byte] &
           (1 << ((index_bit - 1 + 8) % 8)))) {
        stemptsfstream << x[pcount] << "\t" << y[pcount] << "\t" << z[pcount]
          << endl;
      } else {
        if (outputnonstempts) {
          nonstemptsfstream << x[pcount] << "\t" << y[pcount] << "\t" <<
            z[pcount] << "\t" << NONSTEMPTSID << endl;
        }
      }
    }
  } else {
    for (size_t pcount = 0; pcount < np; pcount++) {
      ...
      currentypix =
        static_cast <
        unsigned int >(floor ((y[pcount] - deminfo[3]) / deminfo[4]));
      currentypix = currentypix > 0 ? currentypix : 0;
      currentypix =
        static_cast < unsigned int >(currentypix <
                                     deminfo[1] ? currentypix : deminfo[1]);
      currentxpix =
        static_cast <
        unsigned int >(floor ((x[pcount] - deminfo[2]) / deminfo[4]));
      currentxpix = currentxpix > 0 ? currentxpix : 0;
      currentxpix =
        static_cast < unsigned int >(currentxpix <
                                     deminfo[0] ? currentypix : deminfo[0]);

      if (dem
          [MatrixLZ::
           sub2ind (currentypix, currentxpix, dim_dem[0], dim_dem[1],
                    MatrixLZ::ColMajor)] == deminfo[5]) {
        if (outputnonstempts) {
          nonstemptsfstream << x[pcount] << "\t" << y[pcount] << "\t" <<
            z[pcount] << "\t" << NONSTEMPTSID << endl;
        }
      ...
      topz =
        dem[MatrixLZ::
            sub2ind (currentypix, currentxpix, dim_dem[0], dim_dem[1],
                     MatrixLZ::ColMajor)] + maxstemlength;
...
mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
  ...
  if (!(mxIsNumeric (prhs[3]))) {
    ...
    if (ndim != 2) {
      ...
      if (!(dims[0] == 3 && dims[1] == 3)) {
        mexErrMsgTxt
          ("p2rInfo: numbers of elements in dimensions are not right!");
      }
      //mxFree(dims);
    }
  }
  ...
  if (!(mxIsNumeric (prhs[5]))) {
    ...
    if (!
        (plength[0] == plength[1] && plength[0] == plength[2]
         && plength[1] == plength[2])) {
      mexErrMsgTxt ("the lengths of x, y, z are not consistent!");
    } else if (!(ivlength == plength[0])) {
      mexErrMsgTxt
        ("the length of IndexVector is not consistent with point number!");
    }
  ...
  double *VoxelArray;
  double *voxelarray_yrow, *voxelarray_xcol, *voxelarray_zvertica,
    *voxelarray_pnum;
  double *indexvector;
  ...
  if (!(CppStemExtraction (x, y, z,
                           ...
                           maxx, maxy, maxz,
                           cellx, celly, cellz,
                           voxelarray_yrow, voxelarray_xcol,
                           voxelarray_zvertica, voxelarray_pnum, indexvector,
                           zflag, maxstemlength, dem, dim_dem, deminfo,
                           stemptsfile, nonstemptsfile, ivlength, voxelnum))
    ) {
    mxFree (stemptsfile);
