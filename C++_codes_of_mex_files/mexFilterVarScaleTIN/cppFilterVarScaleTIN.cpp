...
void
cppFilterVarScaleTIN (string InPtsPathName, string InPtsFileName,
                      ...
                      string NonGroundPtsFileName, string EachScalePathName)
{
  /*
  ...
  if (NonGroundPtsFileName != "\0") {
    ...
    if (!NonGroundPtsFile) {
      strErrMsg.clear ();
      strErrMsg =
        "mexFilterVarScaleTIN, failed to open the file of non-ground points: \""
        + tempstring + "\".";
  ...
  if (scale.size () != door.size ()) {
    mexErrMsgTxt
      ("mexFilterVarScaleTIN: the lengths of scale and door are not compatible!");
  }
  if (xlim.size () != 2) {
    mexErrMsgTxt
      ("mexFilterVarScaleTIN: xlim does not contain two elements!");
  }
  if (ylim.size () != 2) {
    mexErrMsgTxt
      ("mexFilterVarScaleTIN: ylim does not contain two elements!");
  }
  ...
  GroundPtsIndFileName =
    GroundPtsPathName + filesep + GroundPtsFileName + ".lnum";
  ...
  for (int si = 0; si < (int) scale.size (); si++) {
    ...
    if (!OldPtsFile) {
      strErrMsg.clear ();
      strErrMsg =
        "mexFilterVarScaleTIN, during search lowest point: failed to open the file: \""
        + OldPtsFileName + "\".";
      mexErrMsgTxt (strErrMsg.c_str ());
    }
    ...
    for (int i = 0; i < xnum; i++) {
      ...
      if (GroundGridX[i] == NULL || GroundGridY[i] == NULL
          || GroundGridZ[i] == NULL) {
        mexErrMsgTxt ("mexFilterVarScaleTIN: No enough memory!");
      }
    }
    ...
    while (1) {
      ...
      if (tempz < GroundGridZ[tempxi][tempyi]) {
        if (GroundGridZ[tempxi][tempyi] == numeric_limits <
            double >::infinity ()) {
          ValidLPNum += 1;
        }
    ...
    int temp = 0;
    double MaxLPZ = -1 * numeric_limits < double >::infinity (), MinLPZ =
    ...
    if (si == (int) scale.size () - 1) {
      if (isnan (CutH)) {
        if (isnan (CalCutH[0]) || isnan (CalCutH[1])) {
          mexErrMsgTxt
            ("mexFilterVarScaleTIN: either CutH or CalCutH must be numeric values, not NaN, not empty or not Inf.");
        }
    ...
    if (!NewPtsFile) {
      strErrMsg.clear ();
      strErrMsg =
        "mexFilterVarScaleTIN, failed to open the newly created file: \"" +
        NewPtsFileName + "\".";
      mexErrMsgTxt (strErrMsg.c_str ());
    }
    ...
    double *ptrdblQueryPX, *ptrdblQueryPY, *ptrdblQueryPZ, *ptrdblSI,
      *ptrdblTIN;
    ...
    for (int i = 0; i < BlockNum; i++) {
      ...
      for (int j = 0; j < BlockSize; j++) {
        ...
        linestringstream.clear ();
        linestringstream.str (linestr);
        linestringstream >> ptrdblQueryPX[j] >> ptrdblQueryPY[j] >>
          ptrdblQueryPZ[j];
      }
      ...
      for (int j = 0; j < BlockSize; j++) {
        ...
        if (mxIsNaN (ptrdblSI[j])) {
          ...
          TriVertex[0] =
            (int) ptrdblTIN[MatrixLZ::
                            sub2ind (SI, 0, TriNum, 3, MatrixLZ::ColMajor)];
          TriVertex[1] =
            (int) ptrdblTIN[MatrixLZ::
                            sub2ind (SI, 1, TriNum, 3, MatrixLZ::ColMajor)];
          TriVertex[2] =
            (int) ptrdblTIN[MatrixLZ::
                            sub2ind (SI, 2, TriNum, 3, MatrixLZ::ColMajor)];

          planeA =
            (ptrdblLowestPtsY[TriVertex[1] - 1] -
             ptrdblLowestPtsY[TriVertex[0] -
                              1]) * (ptrdblLowestPtsZ[TriVertex[2] - 1] -
                                     ptrdblLowestPtsZ[TriVertex[0] - 1])
            - (ptrdblLowestPtsY[TriVertex[2] - 1] -
               ptrdblLowestPtsY[TriVertex[0] -
                                1]) * (ptrdblLowestPtsZ[TriVertex[1] - 1] -
                                       ptrdblLowestPtsZ[TriVertex[0] - 1]);
          planeB =
            (ptrdblLowestPtsX[TriVertex[1] - 1] -
             ptrdblLowestPtsX[TriVertex[0] -
                              1]) * (ptrdblLowestPtsZ[TriVertex[2] - 1] -
                                     ptrdblLowestPtsZ[TriVertex[0] - 1])
            - (ptrdblLowestPtsX[TriVertex[2] - 1] -
               ptrdblLowestPtsX[TriVertex[0] -
                                1]) * (ptrdblLowestPtsZ[TriVertex[1] - 1] -
                                       ptrdblLowestPtsZ[TriVertex[0] - 1]);
          planeB *= -1;
          planeC =
            (ptrdblLowestPtsX[TriVertex[1] - 1] -
             ptrdblLowestPtsX[TriVertex[0] -
                              1]) * (ptrdblLowestPtsY[TriVertex[2] - 1] -
                                     ptrdblLowestPtsY[TriVertex[0] - 1])
            - (ptrdblLowestPtsX[TriVertex[2] - 1] -
               ptrdblLowestPtsX[TriVertex[0] -
                                1]) * (ptrdblLowestPtsY[TriVertex[1] - 1] -
                                       ptrdblLowestPtsY[TriVertex[0] - 1]);
          planeD =
            planeA * ptrdblLowestPtsX[TriVertex[0] - 1] +
            planeB * ptrdblLowestPtsY[TriVertex[0] - 1] +
            planeC * ptrdblLowestPtsZ[TriVertex[0] - 1];
          planeD *= -1;
          ...
          NormalDist =
            fabs (planeA * tempx + planeB * tempy + planeC * tempz + planeD);
          NormalDist =
            NormalDist / sqrt (planeA * planeA + planeB * planeB +
                               planeC * planeC);
          ...
          if (si == (int) scale.size () - 1) {
            if (NormalDist < door[si] && tempz < CutH) {
              ...
              if (OutNonGroundPtsFlag) {
                NonGroundPtsFile << tempx << "\t" << tempy << "\t" << tempz <<
                  endl;
              }
            }
          } else {
