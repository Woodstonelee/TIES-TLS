...
mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
  ...
  if (nrhs < 9) {
    mexErrMsgTxt
      ("mexFilterVarScaleTIN: At least 9 arguments are required as inputs!");
  }
  if (nrhs > 13) {
    mexErrMsgTxt
      ("mexFilterVarScaleTIN: At most 13 arguments are required as inputs!");
  }
  if (nlhs > 1) {
    mexErrMsgTxt
      ("mexFilterVarScaleTIN: at most 1 output argument is allowed!");
  }
  ...
  string NonGroundPtsPathName ("\0"), NonGroundPtsFileName ("\0"),
    EachScalePathName ("\0");
  double *tempptrdbl;
  int NumElem;
  ...
  if (nrhs > 8) {
    ...
    if (!mxIsChar (prhs[2])) {
      mexErrMsgTxt
        ("mexFilterVarScaleTIN: GroundPtsPathName is not a string!");
    } else {
    ...
    if (!mxIsChar (prhs[3])) {
      mexErrMsgTxt
        ("mexFilterVarScaleTIN: GroundPtsFileName is not a string!");
    } else {
    ...
    if (!mxIsNumeric (prhs[8])) {
      ...
      if (mxIsEmpty (prhs[8]) || mxIsNaN (*tempptrdbl)) {
        ...
        if (nrhs < 9) {
          mexErrMsgTxt
            ("mexFilterVarScaleTIN: CutH is empty or NaN, CalCutH must be given to determine the CutH!");
        }
      } else {
        CutH = tempptrdbl[0];
      }
  ...
  if (nrhs > 9) {
    if (!mxIsNumeric (prhs[9])) {
      ...
      if (NumElem != 2) {
        mexErrMsgTxt
          ("mexFilterVarScaleTIN: CalCutH must be a two-elements vector, one is cell size along the Z axis and the other is the coefficient.");
      }
      tempptrdbl = mxGetPr (prhs[9]);
      CalCutH.assign (NumElem, 0);
      if (mxIsEmpty (prhs[8]) || mxIsNaN (tempptrdbl[0])
          || mxIsNaN (tempptrdbl[1])) {
        for (int i = 0; i < NumElem; i++) {
  ...
  if (nrhs > 10) {
    if (nrhs < 12) {
      mexErrMsgTxt
        ("mexFilterVarScaleTIN: NonGroundPtsPathName and NonGroundPtsFileName must be both given!");
    }

    if (!(mxIsEmpty (prhs[10]) || mxIsEmpty (prhs[11]))) {
      if (!mxIsChar (prhs[10])) {
        mexErrMsgTxt
          ("mexFilterVarScaleTIN: NonGroundPtsPathName is not a string!");
      } else {
      ...
      if (!mxIsChar (prhs[11])) {
        mexErrMsgTxt
          ("mexFilterVarScaleTIN: NonGroundPtsFileName is not a string!");
      } else {
  ...
  if (nrhs > 12) {
    if (!mxIsEmpty (prhs[12])) {
      if (!mxIsChar (prhs[12])) {
        mexErrMsgTxt
          ("mexFilterVarScaleTIN: EachScalePathName is not a string!");
      } else {
