// Same with cppDetEstCircle3.cpp except for modifying the ROI of points. 
// ROI is determined with the same method as cppDetEstCircle2.cpp
...
/*	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	Used to create mexDetEstCircle_Complete.mexw32.
	!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
...
bool CppArcInfo(vector<double> x, vector<double> y, vector<double> circle, vector<double> &ArcInfo)
{
				...
				vector<bool> &ROIimage, vector<double> &ROIxpix, vector<double> &ROIypix)
{
	...
	for (unsigned int i=0; i<xpix.size(); i++)
	{
		...
		if ( dis<=maxR && dis>=minR )
		{
			...
			ROIxpix.push_back(xpix[i]);
			ROIypix.push_back(ypix[i]);
		}		
	}	
					 ...
					 vector<double> &DebugInfo)
{
	...
	while ( centerchange>epsion/cellsize || radiuschange>epsion/cellsize )
	{
		...
		// mark ROI
		ROIimage.clear();
		if ( !( CppMarkROI(edgeImage, dim_img, finalHTcircle, xpix, ypix, sigmacoef, finalRsigma, ROIimage, ROIxpix, ROIypix)
			) )
		{
#ifdef MATLABPRINT
